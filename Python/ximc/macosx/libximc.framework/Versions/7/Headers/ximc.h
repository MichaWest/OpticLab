...
/** @cond DO_NOT_WANT */
#define LIBXIMC_VERSION 2.14.14
/** @endcond */


/** @cond DO_NOT_WANT */
#define LIBXIMC_PROTOCOL_VERSION 20.7
/** @endcond */
...
#define STATE_CTP_ERROR                0x0000080 	/**< \english Control position error(is only used with stepper motor). Flag is set when encoder position and step position are too far apart. \endenglish \russian Контроль позиции нарушен(используется только с шаговым двигателем). Флаг устанавливается, когда положение энкодера и положение шага слишком далеки друг от друга. \endrussian */
...
#define STATE_ENGINE_RESPONSE_ERROR    0x0800000 	/**< \english Error response of the engine control action. Motor control algorithm failure means that it can't define the correct decisions with the feedback data it receives. Single failure may be caused by mechanical problem. A repeating failure can be caused by incorrect motor settings. \endenglish \russian Ошибка реакции двигателя на управляющее воздействие. Отказ алгоритма управления двигателем означает, что он не может определять правильные решения с помощью полученных данных обратной связи. Единичный отказ может быть вызван механической проблемой. Повторяющийся сбой может быть вызван неправильной настройкой двигателя. \endrussian */
...
/** 
	...
	*/
	typedef struct
	{
		unsigned int DHCPEnabled;	/**< \english Indicates method to get the IP-address. It can be: 0 — static, 1 — DHCP \endenglish \russian Определяет способ получения IP-адреса каналов. Может принимать значения: 0 — статически, 1 — через DHCP \endrussian */
		unsigned int IPv4Address[4];	/**< \english IP-address of the device in format x.x.x.x. It must be specified as four numbers separated by spaces, not dots \endenglish \russian IP-адрес устройства в формате x.x.x.x. Он должен задаваться в виде четырёх чисел, разделённых пробелами, а не точками \endrussian */
		unsigned int SubnetMask[4];	/**< \english Mask of the subnet in format x.x.x.x. The value must be specified as four numbers separated by spaces, not dots \endenglish \russian Маска подсети в формате x.x.x.x. Значение должно задаваться в виде четырёх чисел, разделённых пробелами, а не точками \endrussian */
		unsigned int DefaultGateway[4];	/**< \english Default value of the gateway in format x.x.x.x. The value must be specified as four numbers separated by spaces, not dots \endenglish \russian Шлюз сети по умолчанию в формате x.x.x.x. Значение должно задаваться в виде четырёх чисел, разделённых пробелами, а не точками \endrussian */
	} network_settings_t;
...
/** 
	...
	*/
	typedef struct
	{
		unsigned int UserPassword[20];	/**< \english Password-string to web-page that user can change with usb command or via web-page. \endenglish \russian Строчка-пароль для доступа к веб-странице, который пользователь может поменять с помощью USB команды или на веб-странице. \endrussian */
	} password_settings_t;
